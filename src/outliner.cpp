...
glm::vec2 Move::sample(float t) const { return (1 - t) * p0 + t * p1; }
...
glm::vec2 Outline::sample(float t) const {
  size_t i = static_cast<size_t>(t * m_segments.size());
  glm::vec2 out;
  std::visit(
      [&out, &t](auto &arg) {
...
FT_Outline_Funcs funcs = (FT_Outline_Funcs){
    .move_to =
        [](const FT_Vector *to, void *user) {
          ...
          // Replace moves with lines so the text is a continuous shape
...
Outline Outliner::outline(std::string_view text) {
  ...
  for (size_t i = 0; i < text.size(); i++) {
    ...
    UserData user = {.segments = segments, .pen = pen};
